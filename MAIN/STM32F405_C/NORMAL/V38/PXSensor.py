...
class PxHub():
    ...
    def SetupFromRFData(self, data=None, sNo=int, dateTime=None):
        ...
        px1sNo = px2sNo = sNo
        direction = (int((data[4] << 8) | data[5]) - int((data[12] << 8) | data[13])) >= (int((data[10] << 8) | data[11]) - int((data[18] << 8) | data[19]))
        ...
        if bool((data[2] >> 0) & 0x01):
            self.px1 = Px(seatNumber=px1sNo, seatBeltPolarity=bool((data[2] >> 2) & 0x01), baseLine=int((data[10] << 8) | data[11]), seatBeltCount=0, seatCount=0, cableErrorCount=0)
            self.features.append(self.px1.features)
            sNo += 1
        else:
            self.features.append([])
        if bool((data[2] >> 1) & 0x01):
            self.px2 = Px(seatNumber=px2sNo, seatBeltPolarity=bool((data[2] >> 3) & 0x01), baseLine=int((data[18] << 8) | data[19]), seatBeltCount=0, seatCount=0, cableErrorCount=0)
        ...
        self.features.append(0)
        self.features.append(-int(data[22] // 2))
        #have to be in order
    ...
    def SetupFromJson(self, json=dict()):  
        ...
        self.features.append(json[key.hub.loopCount])
    ...
    def Process(self, data=None):
        ...
        if bool(data[21] & 0x01):
            for x in range(4):
                if self.px1:
                    self.px1.Calibration(val=int((data[ 4 + (x << 1)] << 8) | data[ 5 + (x << 1)]))
                if self.px2:
                    self.px2.Calibration(val=int((data[12 + (x << 1)] << 8) | data[13 + (x << 1)]))
            ...
            self.features[key.hub.resetCount] += 1 if self.resetCounterStatus == False and bool((data[2] >> 7) & 0x01) else 0
            self.resetCounterStatus = bool((data[2] >> 7) & 0x01)
            self.features[key.hub.battery] = int((((data[2] >> 4) & 0x07)  * 100) // 7)
            self.features[key.hub.rssi] = -int(data[22] // 2)
            self.features[key.hub.loopCount] = int(data[3])
        else:
            self.features[key.hub.crcErrorCount] += 1
            self.features[key.hub.rssi] = -(data[22] // 2)
