...
class PxHub():
    ...
    def SetupFromRFData(self, data=None, sNo=int, dateTime=None):
        ...
        if bool((data[3] >> 0) & 0x01):
            self.px1 = Px(seatNumber=px1sNo, calibrationStatus=True, seatBeltPolarity=bool((data[3] >> 2) & 0x01), baseLine=int(((data[4] << 8) & 0xFF00) | ((data[5] << 0) & 0x00FF)), seatBeltCount=0, seatCount=0, cableErrorCount=0, calibrationCount=0)
            self.features.append(self.px1.features)
            sNo += 1
        else:
            self.features.append([])
        if bool((data[3] >> 1) & 0x01):
            self.px2 = Px(seatNumber=px2sNo, calibrationStatus=True, seatBeltPolarity=bool((data[3] >> 3) & 0x01), baseLine=int(((data[6] << 8) & 0xFF00) | ((data[7] << 0) & 0x00FF)), seatBeltCount=0, seatCount=0, cableErrorCount=0, calibrationCount=0)
        ...
        self.features.append(0)
        self.features.append(int((((data[3] >> 4) & 0x03) * 100) // 3))
    ...
    def Process(self, data=None):
        ...
        if bool(data[9] & 0x01):
            ...
            self.features[key.hub.battery] = int((((data[3] >> 4) & 0x03)  * 100) // 3)
