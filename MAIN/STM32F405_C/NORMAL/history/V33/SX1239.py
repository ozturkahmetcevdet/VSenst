    ...
    def Receive(self):
        if self.RFState == STP:
            ...
            for _ in range(6): # FIFO size is 66 byte
                ...
                if (readVal[0] != 0x00 or readVal[1] != 0x00 or readVal[2] != 0x00) and ((readVal[0] != readVal[1]) or (readVal[1] != readVal[2]) or (readVal[0] != readVal[2])):
                    crcE = readVal[3]
                    readVal = readVal[:3] + readVal[4:]
                    readVal += self.CheckCRC(readVal, crcE)
                    readVal += self.Read(0x24, 1)
                    self.buffer.append(readVal[0:])
                else:
                    break
            self.RFState = BSY

    def CheckCRC(self, data=0x00, crcE=0x00):
        crc = 0xDB
        for i in range(len(data) - 1):
            crc ^= data[i]

        return  b'\x01' if (crc == (data[len(data) - 1])) and (crc == crcE) else b'\x00'
