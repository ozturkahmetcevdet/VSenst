...
class Sensor():
    ...
    def Calibration(self, p1Res=0, p2Res=0):
        ...
        if p1Res < (self.Prox1_ADCReferance >> 3) or p2Res < (self.Prox2_ADCReferance >> 3):
            self.Prox1_Calibrate = self.Prox2_Calibrate = True
        ...
        if self.Prox1_CurrentResolation < self.Prox1_NegativeThreshold:
            ...
            if self.Prox1_Counter > (self.Prox1_CounterLimit >> 2) and self.Prox1_PeakValue < self.Prox1_FilterValue:
                ...
                self.Prox1_CurrentResolation = 0
            if self.Prox1_Counter == 0 and self.Prox1_PeakValue < (Proximity.Default.Threshold if self.SeatCount == 2 else (Proximity.Default.Threshold / Proximity.Default.thresholdDivider)):
            ...
            self.Prox1_Calibrate = False if self.Prox1_Counter > (self.Prox1_CounterLimit >> 1) else self.Prox1_Calibrate
            self.Prox1_Counter = 0 if self.Prox1_Calibrate == False else self.Prox1_Counter
        self.Prox1_CurrentResolation = 0 if self.Prox1_CurrentResolation < 0 else self.Prox1_CurrentResolation
        ...
        if self.Prox2_CurrentResolation < self.Prox2_NegativeThreshold:
            ...
            if self.Prox2_Counter > (self.Prox2_CounterLimit >> 2) and self.Prox2_PeakValue < self.Prox2_FilterValue:
                ...
                self.Prox2_CurrentResolation = 0
            if self.Prox2_Counter == 0 and self.Prox2_PeakValue < (Proximity.Default.Threshold if self.SeatCount == 2 else (Proximity.Default.Threshold / Proximity.Default.thresholdDivider)):
            ...
            self.Prox2_Calibrate = False if self.Prox2_Counter > (self.Prox2_CounterLimit >> 1) else self.Prox2_Calibrate
            self.Prox2_Counter = 0 if self.Prox2_Calibrate == False else self.Prox2_Counter
        self.Prox2_CurrentResolation = 0 if self.Prox2_CurrentResolation < 0 else self.Prox2_CurrentResolation
        ...
        if self.SeatCount == 2: 
            ...
            if self.Prox1_CurrentResolation > (Proximity.Default.Threshold / Proximity.Default.thresholdDivider):
                calc = self.Prox1_CurrentResolation * 0.4
                self.Prox1_Threshold = calc if self.Prox1_Threshold < calc else self.Prox1_Threshold
                
            if self.Prox1_Threshold != (Proximity.Default.Threshold / Proximity.Default.thresholdDivider) and self.Prox1_CurrentResolation < self.Prox1_Threshold:
                self.Prox1_ADCReferance += self.Prox1_CurrentResolation
                self.Prox1_CurrentResolation = 0
            ...
            if self.CheckCRC(data) == True:
                ...
                if checkFlag is True:
                    ...
                    if (((data[3] >> 0) & 0x01) == 1) and (data[6] | (data[5] << 8)) > (data[8] | (data[7] << 8)):
                        if ((data[4] >> 0) & 0x01) == 1:
                            self.CreateSensorObject(data[:3], True, True, [3, 5, Proximity.Default.Thresholde], True, [4, 7, Proximity.Default.Threshold])
                        else:
                            self.CreateSensorObject(data[:3], True, True, [3, 5, Proximity.Default.Threshold], False, [4, 7, Proximity.Default.Threshold])
                        ...
                        if ((data[3] >> 0) & 0x01) == 1:
                            ...
                            self.CreateSensorObject(data[:3], True, True, [4, 7, Proximity.Default.Threshold], False, [3, 5, Proximity.Default.Threshold])
