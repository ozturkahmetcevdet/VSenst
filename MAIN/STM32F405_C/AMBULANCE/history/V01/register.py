...
class Sensor():
    ...
    def __init__(self, ID=bytearray(3), IsSensorHasID=False, Prox1Active=False, Prox1Config=bytearray(5), Prox2Active=False, Prox2Config=bytearray(5)):
        ...
        self.Prox1_Count = 0
        self.Prox1_Direction = Direction.ZERO
        ...
        self.Prox2_Count = 0
        self.Prox2_Direction = Direction.ZERO
    ...
    def DataIn(self, data=bytearray(InComingDataSize)):
        ...
        Prox1ResolationCurrentValue = 0 if self.Prox1_IsSeatActive == False else data[self.Prox1_ResolationAddressInArray + 1] | (data[self.Prox1_ResolationAddressInArray] << 8)
        Prox2ResolationCurrentValue = 0 if self.Prox2_IsSeatActive == False else  data[self.Prox2_ResolationAddressInArray + 1] | (data[self.Prox2_ResolationAddressInArray] << 8)

        self.Prox1_Direction = ((data[self.Prox1_ConfigAddressInArray] >> 2) & 0x03)
        self.Prox2_Direction = ((data[self.Prox2_ConfigAddressInArray] >> 2) & 0x03)
...
class DataBase():
    ...
    def Process(self, fullData=[]):
        ...
        for item in self.SensorList:
            if item.Prox2_SeatHasSensor == True:
                ...
                directionStringValue = "ZERO"
                if item.Prox2_Direction == Direction.DOWN:
                    directionStringValue = "DOWN"
                elif item.Prox2_Direction == Direction.UP:
                    directionStringValue = "UP"

                debugLog += "{tc0}{0:2}-{tc1} ID:{id_c1}{1:3}{id_c2}, Sensor Resolation:{re_c1}{2:4}{re_c2}, Sensor Cable:{se_c1}{3:1}{se_c2}, Seat Belt:{be_c1}{4:1}{be_c2}, Count:{co_c1}{5:4}{co_c2}, Direction:{di_c1}{6:4}{di_c2} --> RF Received Data Count:{rf_c1}{7:6}{rf_c2}, Battery:{bt_c1}%{8:3}{bt_c2}\n\r"     \
                            .format(itemCount, binascii.hexlify(item.ID), item.Prox2_CurrentResolation, item.Prox2_IsSeatActive, item.Prox2_IsSeatHasBelt, item.Prox2_Count, directionStringValue, item.InComingDataCount, item.BatteryMeasurementPercent   \
                                                ...
                                                , di_c1 = TextColour.HEADER                                                                 , di_c2 = TextColour.ENDC                          \
                ...
                lcdDebugLog += "{0:2}- Res:{2:4}, Se:{3:1}, Be:{4:1}, Co:{5:2}, RCo:{6:4}"     \
                            .format(itemCount, binascii.hexlify(item.ID), item.Prox2_CurrentResolation, item.Prox2_IsSeatActive, item.Prox2_IsSeatHasBelt, item.Prox2_Count, item.InComingDataCount)
            if item.Prox1_SeatHasSensor == True:
                itemCount += 1
                
                directionStringValue = "ZERO"
                if item.Prox1_Direction == Direction.DOWN:
                    directionStringValue = "DOWN"
                elif item.Prox1_Direction == Direction.UP:
                    directionStringValue = "UP"

                debugLog += "{tc0}{0:2}-{tc1} ID:{id_c1}{1:3}{id_c2}, Sensor Resolation:{re_c1}{2:4}{re_c2}, Sensor Cable:{se_c1}{3:1}{se_c2}, Seat Belt:{be_c1}{4:1}{be_c2}, Count:{co_c1}{5:4}{co_c2}, Direction:{di_c1}{6:4}{di_c2} --> RF Received Data Count:{rf_c1}{7:6}{rf_c2}, Battery:{bt_c1}%{8:3}{bt_c2}\n\r"     \
                            .format(itemCount, binascii.hexlify(item.ID), item.Prox1_CurrentResolation, item.Prox1_IsSeatActive, item.Prox1_IsSeatHasBelt, item.Prox1_Count, directionStringValue, item.InComingDataCount, item.BatteryMeasurementPercent   \
                                                , tc0   = TextColour.BOLD + TextColour.OKCYAN                                               , tc1   = TextColour.ENDC                          \
                                                , id_c1 = TextColour.OKBLUE                                                                 , id_c2 = TextColour.ENDC                          \
                                                ...
                                                , bt_c1 = TextColour.OKGREEN if item.BatteryMeasurementPercent > 20 else TextColour.FAIL    , bt_c2 = TextColour.ENDC)
                lcdDebugLog += "{0:2}- Res:{2:4}, Se:{3:1}, Be:{4:1}, Co:{5:2}, RCo:{6:4}"     \
                            .format(itemCount, binascii.hexlify(item.ID), item.Prox1_CurrentResolation, item.Prox1_IsSeatActive, item.Prox1_IsSeatHasBelt, item.Prox1_Count, item.InComingDataCount)
        print(debugLog)
        return lcdDebugLog + "\""
    ...
    def ClearAllData(self):
        ...
        try:
            ...
            print("Could not remove file. ---{0}".format(name))
        except:
            print("Unexpected error!")
            raise
