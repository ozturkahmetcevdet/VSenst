...
peripheral.LCDCOMMAND = LCDCOMMAND
    ...
    if value == True:
        ...
        if Operation.Mode.Value == Operation.Mode.Sleep:
            ...
            for i in range(LCDCOMMAND.Seat.NumberOfSeats):
                ...
                LCDCOMMAND.Belt.Buffer[i] = LCDCOMMAND.Belt.Registered
            LCDCOMMAND.Page.Buffer = LCDCOMMAND.Page.Main
            LCDCOMMAND.Ignition.Buffer = LCDCOMMAND.Ignition.On
            ...
            if item.Prox2_SeatHasSensor == True:
                if item.Prox2_IsSeatActive == True:
                    if item.Prox2_IsSeatHasPassanger == True:
                        LCDCOMMAND.Seat.Buffer[c] = LCDCOMMAND.Seat.Full
                        if item.Prox2_IsSeatHasBelt == True:
                            LCDCOMMAND.Belt.Buffer[c] = LCDCOMMAND.Belt.BeltOnSeatOn 
                        else:
                            alarmBuzzerFlag = True
                            peripheral.ToggleAddObject(c, LCDCOMMAND.Belt.BeltOffSeatOn, LCDCOMMAND.Belt.Registered)
                        
                    else:
                        LCDCOMMAND.Seat.Buffer[c] = LCDCOMMAND.Seat.Registered
                        LCDCOMMAND.Belt.Buffer[c] = LCDCOMMAND.Belt.Registered
                c += 1
