...
void Graphics::Loop(bool enable)
{
  ...
  for(int j = 0; j < sizeof(this->SEAT_CurrentImage); j++)
  {
    ...
    if(this->SEAT_WheelChairActive[j] && (this->SEAT_CurrentImage[j] < 5))
    {
      this->SetBitmap(this->SEAT_PositionXY[j][0], this->SEAT_PositionXY[j][1], 24, 24, this->SEAT_WheelChairList[this->SEAT_CurrentImage[j] - 1], this->SEAT_ImageProcess[j]);
    }
  }
}
