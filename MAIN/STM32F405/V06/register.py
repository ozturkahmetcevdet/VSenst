...
class Sensor():
    ...
    def DataIn(self, data=bytearray(16)):
        ...
        self.Calibration(data[self.Prox1_ResolationAddressInArray + 1] | (data[self.Prox1_ResolationAddressInArray] << 8), data[self.Prox2_ResolationAddressInArray + 1] | (data[self.Prox2_ResolationAddressInArray] << 8))
        ...
        if self.Prox1_IsSeatActive is True:
            if data[self.Prox1_ResolationAddressInArray + 1] | (data[self.Prox1_ResolationAddressInArray] << 8) > Proximity.Default.MaxLineThreshold:
                self.Prox1_CurrentResolation = data[self.Prox1_ResolationAddressInArray + 1] | (data[self.Prox1_ResolationAddressInArray] << 8) - Proximity.Default.MaxLineThreshold
            else:
                self.Prox1_CurrentResolation = data[self.Prox1_ResolationAddressInArray + 1] | (data[self.Prox1_ResolationAddressInArray] << 8)
            print("p1:" + str(self.Prox1_CurrentResolation))
        ...
        if self.Prox2_IsSeatActive is True:
            if data[self.Prox2_ResolationAddressInArray + 1] | (data[self.Prox2_ResolationAddressInArray] << 8) > Proximity.Default.MaxLineThreshold:
                self.Prox2_CurrentResolation = data[self.Prox2_ResolationAddressInArray + 1] | (data[self.Prox2_ResolationAddressInArray] << 8) - Proximity.Default.MaxLineThreshold
            else:
                self.Prox2_CurrentResolation = data[self.Prox2_ResolationAddressInArray + 1] | (data[self.Prox2_ResolationAddressInArray] << 8)
            print("p2:" + str(self.Prox2_CurrentResolation))
    ...
    def Calibration(self, p1Res=0, p2Res=0):
        if p1Res != 0 and p1Res > p2Res:
            Proximity.Default.constVal = Proximity.Default.multiple * p1Res / Proximity.Default.divider
            if p2Res != 0:
                value = p1Res - p2Res
                if value > Proximity.Default.constVal:
                    value -= Proximity.Default.constVal
                    if value > Proximity.Default.constVal:
                        self.Prox1_Threshold = p1Res - value
            ...
            if p1Res != 0:
                ...
                if value > Proximity.Default.constVal:
                    value -= Proximity.Default.constVal
                    if value > Proximity.Default.constVal:
                        self.Prox2_Threshold = p2Res - value
